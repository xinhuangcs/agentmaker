...
from ..runtime.execution.run_context import (current_run_id, current_step, new_run_id, reset_run,
                                             snapshot_usage, start_run)
...
class BaseAgent:
    ...
    def __init__(self, name: str, llm: LLMClient, system_prompt: Optional[str] = None, *,
        ...
        # Prompt registry (PromptRegistry, see agentmaker.prompts): this strategy's built-in
        # prompts are all fetched from it by key. If not passed, a copy of DEFAULT_PROMPTS is made
        # (isolated per instance, so update_prompts no longer mutates the global). The strategy passes
        # this same copy verbatim to its own Harness and internal sub-agents, so a single
        # update_prompts takes effect across this agent's whole chain, isolated across agents.
        self.prompts = prompts if prompts is not None else DEFAULT_PROMPTS.copy()
        self._harness_cfg = harness_config or HarnessConfig()   # Cross-cutting knobs (_make_harness assembles from this; hooks/prompts injected automatically by the factory)
        self._sessions: dict = {}      # Per-scope in-process history when there is no session_store; with a store, each run loads from it and does not cache here
        self._scope_locks: "weakref.WeakValueDictionary" = weakref.WeakValueDictionary()   # One async lock per scope (see _scope_lock)
    ...
    async def arun(self, input_text: str, *, scope=None, trace_carrier=None, **kwargs):
        ...
        async with self._scope_lock(scope), self._scaffold(input_text, scope, trace_carrier):
            ...
            await self._mark_checkpoint_completed(scope)   # First mark the lingering checkpoint completed=True: record and clear below are not atomic, so a crash in between leaves a lingering checkpoint recognizable as completed and not re-run
    ...
    def _scope_lock(self, scope) -> asyncio.Lock:
        """Return the async lock for this scope, serializing "the run / resume lifecycle for the same scope" (check pending -> execute -> persist suspend).
        ...
        The lock is created lazily per scope key and stored in a WeakValueDictionary (reclaimed when
        no one holds it, preventing unbounded growth across many scopes; the `async with` holds a
        strong reference while held / awaited, so it is not reclaimed). Nested sub-agents run on
        distinct derived child scopes and take distinct locks, so they never self-deadlock against the
        parent. Only effective within a single process: cross-process needs optimistic locking at the
        checkpoint_store layer (version-column CAS), which is a known boundary (this lock suffices for
        the single-process daemon scenario).
        """
    ...
    async def _guard_pending(self, scope) -> None:
        ...
        if checkpoint_format_version(raw) != CHECKPOINT_FORMAT_VERSION:
            await self.checkpoint_store.aclear(scope=scope)   # An incompatible old checkpoint can no longer be resumed; stop blocking new runs (clear it, pass through)
            return
    ...
    async def _record(self, input_text: str, output, scope) -> None:
        ...
        # A turn's user+assistant pair is persisted at once: with a store it goes through append_many
        # (single transaction), avoiding a half-turn history (see add_messages).
    ...
    async def _suspend(self, state, pending, scope):
        ...
        the strategy's `_adrive`). A single value is normalized into a one-element list (backward compatible).
        """
    ...
    async def _load_execution_state(self, scope):
        ...
        is no store or no checkpoint; when the checkpoint format version does not match the current one
        (most likely a framework upgrade, including old formats missing "v") it automatically clears
        that scope's stale suspended state and raises so the user re-initiates (the "discard on version
        mismatch" policy for transient data; a real old checkpoint missing "v" is caught by it).
        """
        from ..runtime.execution.state import CHECKPOINT_FORMAT_VERSION, ExecutionState, checkpoint_format_version
        if self.checkpoint_store is None:
        ...
        if checkpoint_format_version(raw) != CHECKPOINT_FORMAT_VERSION:
            await self.checkpoint_store.aclear(scope=scope)   # Discard on version mismatch: clear the stale suspended state to avoid misreading and resuming an old format
            raise SessionError(
                "This suspended task's checkpoint format is incompatible with the current framework version (most likely a framework upgrade); it has been cleared automatically. Please re-initiate the task.")
        state = ExecutionState.from_json(raw)
        if state.completed:
            ...
            raise SessionError("This run has completed (history already written); there is no recoverable suspended / unfinished content. The lingering checkpoint has been cleared.")
        return state
    ...
    async def _finish_resume(self, output, state, scope):
        ...
        resume is the continuation of an unfinished run, so it does not re-fire on_run_start; final
        completion / re-suspend is funneled through this method to fire the corresponding hook.
        """
        if self._is_interrupt(output):
            await afire(self.hooks, "on_interrupt", output.pendings, scope=scope)
        ...
        await self._record(state.input_text, output, scope)   # Persist history first (including output guardrails): same order as the run path
        await self._clear_checkpoint(scope)                    # Clear the checkpoint only after persisting succeeds (does not self-clear under defer, left to the parent strategy): a guardrail trip / persistence failure keeps the checkpoint for another resume
    ...
    async def add_messages(self, messages: list[Message], scope=None):
        ...
        if self.session_store is not None:
            ...
            if callable(aappend_many):
                await aappend_many(messages, scope=scope)    # SessionStore subclass: single atomic transaction (the default a* goes via to_thread)
            else:                                            # A duck-typed store that only implements aappend: persist one by one (non-atomic fallback, backward compatible)
                for m in messages:
