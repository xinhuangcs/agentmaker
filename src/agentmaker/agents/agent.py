...
Structured work inside the loop:
    ...
    - call_id collision guard: when a turn's tool_calls have ids that collide with existing keys in the
      decision table (some servers auto-increment within a response and repeat across responses), rewrite
      them to unique ids so an approval cannot be applied to a different action.
...
from agentmaker.core.exceptions import LLMConfigError, LLMResponseError
from agentmaker.runtime.execution import ExecutionState
from agentmaker.runtime.execution.run_context import check_deadline, has_parallel_sensitive_limits
...
class Agent(BaseAgent):
    ...
    async def _adrive(self, state, *, scope, verbose: bool = False, **kwargs):
        ...
        while state.remaining > 0:
            ...
            state.messages.append(self._assistant_tool_message(resp, calls))
    ...
    def _unique_calls(self, calls: list, state) -> list:
        """Prevent approval borrowing: if a turn's tool_calls have ids that collide with existing keys
        ...
        Otherwise a decision for the colliding id could authorize a different action
    ...
    @staticmethod
    def _assistant_tool_message(resp, calls: list) -> dict[str, Any]:
        """Build the assistant history item without dropping provider continuation state."""
        message = dict(getattr(resp, "assistant_message", None) or {})
        message.update({"role": "assistant", "content": resp.content or "", "tool_calls": calls})
        return message
    ...
    async def _run_calls(self, calls, state, decisions, scope, verbose: bool = False):
        ...
        without disturbing the serial branch. After executing each serial call with a decision (a
        ...
        if decisions is not None:
            self._validate_approval_call_ids(calls)
        idx, n = 0, len(calls)
        while idx < n:
            ...
            state.messages.append({"role": "tool", "tool_call_id": call["id"],
                                   "content": self._tool_content(name, params, result)})
    ...
    def _validate_approval_call_ids(self, calls) -> None:
        """Fail closed when a confirmation-required call lacks a unique ID within its batch."""
        if not calls or self.tool_registry is None:
            return
        id_counts = {}
        parsed = []
        for call in calls:
            ...
            parsed.append((call, self._parse_call(call)))
        for call, (name, params, parse_error) in parsed:
            ...
            if not isinstance(call_id, str) or not call_id.strip():
                raise LLMResponseError(
                    f"Cannot request human approval for tool '{name}': the model/provider returned an empty "
                    "tool call ID. Approval decisions require a stable unique ID; use a provider/model that "
                    "supplies tool call IDs, or use an explicit synchronous confirmation callback in a "
                    "non-persistent CLI flow.")
            if id_counts.get(call_id, 0) != 1:
                raise LLMResponseError(
                    f"Cannot request human approval for tool '{name}': tool call ID {call_id!r} appears more "
                    "than once in the same model response. The run was stopped to prevent one approval from "
                    "being applied to multiple actions.")
    ...
    def _parallel_run_len(self, calls, start, decisions) -> int:
        ...
        if has_parallel_sensitive_limits():
            return 0
        k = start
    ...
    def _parallel_eligible(self, call, decisions) -> bool:
        ...
        if tool is None or not getattr(tool, "supports_parallel", False):
            return False
        return not tool.needs_confirmation(params)
    ...
    async def _run_parallel_batch(self, batch, state, decisions, scope, verbose: bool) -> None:
        ...
        for call in batch:
            name, params, perr = self._parse_call(call)
            assert perr is None
            parsed.append((name, cast(dict[str, Any], params)))
        results = await asyncio.gather(*(
            self.harness.aexec_tool(name, params, call_id=c["id"], decisions=decisions)
            for c, (name, params) in zip(batch, parsed)))
        for c, (name, params), result in zip(batch, parsed, results):
            self._collect_discovered(result, state)
            if verbose:
                print(f"🔧  [{name}] {result.text}")
    ...
    def _tool_content(self, name: str, parameters: Optional[dict], result) -> str:
        """The tool-result text fed back to the model: external-content calls
        ...
        everything else is passed through verbatim. Error text is wrapped too, since an external tool's
        failure message can itself be attacker-controlled. Only the copy the LLM sees is wrapped: the
        ToolResponse itself is unchanged (trace / data / _collect_discovered still read the original)."""
        tool = self.tool_registry.get(name) if self.tool_registry is not None else None
        external = (tool.is_external_content(parameters or {})
                    if callable(getattr(tool, "is_external_content", None))
                    else getattr(tool, "external_content", False)) if tool is not None else False
        if external:
    ...
    def _parse_call(
        call: dict[str, Any],
    ) -> tuple[str, dict[str, Any], None] | tuple[str, None, str]:
        ...
        if not isinstance(params, dict):
            return name, None, f"Error: arguments for tool '{name}' must be a JSON object"
    ...
    async def astream_run(self, input_text: str, *, scope: "Optional[Scope]" = None, buffer_output: bool = False,
        ...
        synchronous confirm callback, or is safely rejected when no callback is configured. Use arun when
        suspend semantics are required. History save matches arun: one atomic user + final-assistant turn;
        tool trajectories are not persisted to session history.
        ...
        - `buffer_output=True`: accumulate the full output first, run the output guardrail, and only after it
          passes release it piece by piece. With a tool loop this guards and buffers all streamed text across
          turns before releasing any of it (tools still execute live in between).
            ...
            if tools:
                if not self._supports_streaming_tools():
                    raise LLMConfigError(
                        "This model adapter does not support streaming tool calls (text tool emulation is "
                        "non-streaming). Use await agent.arun(...) / agent.run(...) for tool emulation, or "
                        "switch to a model with native streaming function calling.")
            ...
            if buffer_output:
                async for piece in self.harness.astream_llm(messages, **kwargs):
                    if not isinstance(piece, str):
                        raise LLMResponseError(
                            "A plain-text stream emitted an unexpected terminal response without tools.")
    ...
    async def _astream_tool_loop(self, input_text, scope, tools, *, buffer_output: bool,
        ...
        while state.remaining > 0:
            ...
            if resp is None:
                raise LLMResponseError(
                    "A streaming tool turn ended without the terminal LLMResponse required to carry "
                    "tool calls. Set supports_streaming_tools=False for a text-only stream, or update "
                    "the adapter to emit one terminal LLMResponse after its text deltas.")
            if not resp.tool_calls:
                content = (resp.content or "").strip()
                if content:
        ...
        guard_text = output
        if buffer_output:
            visible_text = "".join(buffered) + (unstreamed_tail or "")
            if visible_text:
                guard_text = visible_text
                if output and output not in visible_text:
                    guard_text = f"{visible_text}\n{output}"
        await self._check_guardrails(self.output_guardrails, guard_text, "output")
        if buffer_output:
            check_deadline()
        ...
        if unstreamed_tail is not None:
            if not buffer_output:
                check_deadline()                                  # the fallback tail was never streamed, so the deadline still gates it
            yield unstreamed_tail                                 # fallback texts reach the consumer as stream content too
        await self.add_messages([Message(input_text, "user"), Message(output, "assistant")], scope)  # One atomic turn save
        await afire(self.hooks, "on_run_end", output, scope=scope)

    def _supports_streaming_tools(self) -> bool:
        """Read the optional streaming-tool capability from the LLM or its adapter."""
        direct = getattr(self.llm, "supports_streaming_tools", None)
        if direct is not None:
            return bool(direct)
        adapter = getattr(self.llm, "_adapter", None)
        if adapter is not None:
            return getattr(adapter, "supports_streaming_tools", True) is not False
    ...
    def stream_run(self, input_text: str, *, scope=None, **kwargs) -> Iterator[str]:
