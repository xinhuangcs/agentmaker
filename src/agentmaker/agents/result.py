...
run / arun / resume / aresume no longer return the bare three-state union `str | structured instance
| Interrupt` (a bare Interrupt is easily mistaken for a string, a silent accident). They return a
single RunResult instead, bundling final output / whether a human-approval interrupt occurred / usage
/ new messages this turn / run_id into one object. A suspension goes from "a bare Interrupt mistaken
for a str" to the explicit `status == "interrupted"` state. This mirrors OpenAI Agents SDK's
RunResult and Pydantic AI's AgentRunResult.
...
class RunResult:
    ...
    def __str__(self) -> str:
        """Let print(result) / f"{result}" show the final output text directly (eases migration: old
        code that used it as a string still gets the answer); a suspended state shows a readable note
