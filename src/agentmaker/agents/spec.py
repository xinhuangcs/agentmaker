...
class _CommonAgentKwargs(TypedDict):
    """Keyword arguments shared by every agent strategy constructor."""
    ...
    prompts: Optional[PromptRegistry]
...
class AgentSpec:
    ...
    model: Optional[Union[str, LLMClient]] = None           # str -> "provider:model" (e.g. "deepseek:deepseek-v4-flash") or a bare provider name / an LLMClient or duck-typed client with chat/stream -> used directly / None -> default LLMClient()
...
def build_agent(spec: AgentSpec) -> BaseAgent:
    ...
    model: str -> `"provider:model"` (e.g. `"deepseek:deepseek-v4-flash"`, split into
        ...
        `LLMClient` instance (or any duck-typed client exposing chat/stream, e.g. a test double) ->
        used directly (lets you pin model / key / base_url); None -> `LLMClient()` default.
    ...
    # Cheap spec validation runs before resolving the LLM, so a config mistake is not masked by an LLM-resolution error.
    if spec.strategy == "react" and (registry is None or not registry.list_tools()):  # react must have tools (both None and an empty list are rejected)
    ...
    if spec.strategy in ("reflection", "plan"):
        _reject(spec, {"compactor"})   # Reflection / Plan trim their own history by window and accept no compactor
    _turns(spec.max_turns, 1)          # validate max_turns; the per-strategy default is applied at construction below
    llm = _resolve_llm(spec.model)
    common: _CommonAgentKwargs = {
        "system_prompt": spec.instructions,
        "tracer": spec.tracer,
        "session_store": spec.session_store,
        "scope": spec.scope,
        "input_guardrails": spec.input_guardrails,
        "output_guardrails": spec.output_guardrails,
        "hooks": spec.hooks,
    ...
    if spec.strategy == "react":
        ...
        common_react: _CommonAgentKwargs = {**common, "system_prompt": _react_system(spec)}
...
def _resolve_llm(model) -> LLMClient:
    """Resolve AgentSpec.model into an LLM client: None -> default / LLMClient (or duck-typed client) -> as-is / str -> split "provider:model" or a bare provider name.
    ...
    treat the string as a bare provider name. A duck-typed client exposing chat/stream (such as a test
    double) is accepted as-is, matching how Agent(...) accepts one. Anything else fails loud.
    """
    if model is None:
        return LLMClient()
    ...
    if callable(getattr(model, "chat", None)) or callable(getattr(model, "stream", None)):
        return model
    raise TypeError(f"AgentSpec.model must be a str ('provider:model' or a bare provider name), an LLMClient, a duck-typed client with chat/stream, or None, got {type(model).__name__}")
