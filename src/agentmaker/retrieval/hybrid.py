...
def _exact_delete(store, ids: List[str], *, scope: Optional[Scope] = None) -> None:
    """Delete via the store's exact-footprint method, or ranged delete for duck-typed stores without one.

    The ranged fallback refuses a fully empty footprint: that range would be the whole store.
    """
    method = getattr(store, "delete_exact", None)
    if method is None:
        require_explicit_scope(scope or Scope(), False, "delete_exact fallback")
        store.delete(ids, scope=scope)
    else:
        method(ids, scope=scope)
...
class HybridRetriever:
    ...
    def add(self, ids: List[str], contents: List[str], *, scope: Optional[Scope] = None,
        ...
        try:
            ...
            try:
                _exact_delete(self.vector_store, ids, scope=scope)  # undo the vector just written: exact delete (== the write footprint), so it does not wrongly delete same-id rows from other scopes
    ...
    def replace(self, old_ids: List[str], new_ids: List[str], contents: List[str], *,
        ...
        if stale:
            self.delete_exact(stale, scope=scope)
    ...
    def delete_exact(self, ids: List[str], *, scope: Optional[Scope] = None) -> None:
        """Delete ids from one exact ownership footprint in both indexes (ranged fallback for stores without exact deletes)."""
        _exact_delete(self.vector_store, ids, scope=scope)
        _exact_delete(self.keyword_index, ids, scope=scope)
