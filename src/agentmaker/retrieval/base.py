...
class VectorStore(ABC):
    ...
    def delete_exact(self, ids: List[str], *, scope: Optional[Scope] = None) -> None:
        """Delete by exact write footprint, defaulting to a ranged-delete fallback.

        The fallback range equals the footprint, so it touches extra rows only when
        sibling footprints share an id inside that range; a fully empty footprint is
        refused (the range would be the whole store). Backends that store the full
        footprint (such as SqliteVecStore) override with a true exact delete.
        """
...
class KeywordIndex(ABC):
    ...
    def delete_exact(self, ids: List[str], *, scope: Optional[Scope] = None) -> None:
        """Delete by exact write footprint, defaulting to a ranged-delete fallback (same contract as VectorStore.delete_exact)."""
        require_explicit_scope(scope or Scope(), False, "delete_exact fallback")
        self.delete(ids, scope=scope)
