...
class OpenAIEmbedder(Embedder):
    ...
    def __init__(self, model: str = "text-embedding-3-small", api_key: Optional[str] = None,
        ...
        if dimensions is not None and dimensions < 1:
            raise RetrievalError(f"dimensions must be >= 1, got {dimensions}")
        if timeout <= 0:
            raise RetrievalError(f"timeout must be > 0, got {timeout}")
        dim = dimensions if dimensions is not None else _OPENAI_DIMS.get(model)
        if not self.api_key:
