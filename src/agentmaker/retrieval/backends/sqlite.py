    ...
    def __init__(self, dim: int, db_path: str = ":memory:", *, table: str = "vec_items",
        ...
        try:
            conn.execute(
                ...
                f"embedding float[{dim}], id text, {scope_cols}{md_cols}, +content text)")
        except sqlite3.Error as e:
            # Normalize to RetrievalError like Fts5KeywordIndex's CREATE (e.g. an illegal dimension float[0], a bad metadata column name)
            raise RetrievalError(f"Failed to create the vector table (is dimension {dim} valid?): {e}") from e
        # Locked-in-parameters open-time self-check: the vec0 dimension / scope columns / declared md columns are fixed on
        # first creation; IF NOT EXISTS silently reuses the old parameters and cannot ALTER them. The direct-construction
        # path (not going through build_sqlite_hybrid's model_id | dimension fingerprint) is backstopped here: if the old
        # database's DDL is missing the currently expected fragments (a swapped embedder changed the dimension, or
        # metadata_columns changed), fail loud and point to a rebuild, rather than silently using the wrong dimension and
        # producing garbage results.
        require_ddl_contains(
            conn, self.table,
            [f"embedding float[{dim}]", *[f"{c} text" for c in scope_column_names()], *[f"md_{k} text" for k in self.metadata_columns]],
...
class Fts5KeywordIndex(KeywordIndex):
    ...
    def __init__(self, db_path: str = ":memory:", *, table: str = "kw_items",
        ...
        # Locked-in-parameters open-time self-check: the fts5 tokenizer / scope columns / md columns are fixed on first
        # creation and cannot be ALTERed. If the old database's DDL is missing the currently expected fragments
        # (metadata_columns changed, or a future tokenize change), fail loud and point to a rebuild rather than silently
        # reusing the old parameters.
        require_ddl_contains(
    ...
    def replace(self, old_ids, new_ids, contents, *, scope=None, metadatas=None):
        ...
        with self._lock:
            try:
                ...
                if stale:
                    self.vector_store.delete(stale, scope=scope)
                    self.keyword_index.delete(stale, scope=scope)
...
def build_sqlite_hybrid(embedder: Embedder, *, db_path: str = ":memory:", reranker: Optional[Reranker] = None,
    ...
    try:
        ...
        conn.close()  # mid-construction failure (e.g. missing sqlite-vec / FTS5 not compiled / fingerprint mismatch): close the already-built connection, do not leak the file handle
        raise
