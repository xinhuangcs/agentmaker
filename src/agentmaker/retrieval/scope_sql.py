...
def scope_store_values(scope: Scope) -> List[str]:
    ...
    Shares the same source as scope_column_names (both iterate _SCOPE_COLS) -> strictly aligned: editing _SCOPE_COLS in
    one place guarantees that, within the code, stored values and column names do not drift (drift would write values
    into the wrong column and leak data across scopes without error). Note: this only holds for newly-created databases.
    _SCOPE_COLS is the persistence schema contract for every SQLite table; adding or removing a dimension is a
    whole-database schema change, and existing old databases must be migrated (each store's open-time self-check will
    block incompatible old databases). See scope.py and doc/retrieval/scope.md.
    """
