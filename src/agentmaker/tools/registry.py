...
import logging
import re
from typing import TYPE_CHECKING, Awaitable, Callable, Dict, Iterable, List, Optional
...
_FunctionResult = str | ToolResponse
_FunctionCallable = Callable[[dict], _FunctionResult | Awaitable[_FunctionResult]]
...
class _FunctionTool(Tool):
    ...
    def __init__(self, func: _FunctionCallable, name: str, description: str,
    ...
    async def arun(self, parameters: dict) -> ToolResponse:
        ...
        if self._is_async:
            ...
            if not inspect.isawaitable(result):
                raise TypeError(f"the async function for tool '{self.name}' returned a non-awaitable result")
            result = await result
            return result if isinstance(result, ToolResponse) else ToolResponse.ok(str(result))
...
class ToolRegistry:
    ...
    def register_function(self, func: _FunctionCallable, name: str, description: str,
    ...
    def register_callable(self, func: Callable, *, name: Optional[str] = None, description: Optional[str] = None,
        ...
        Args:
            ...
            description: Tool description; defaults to the first line of the docstring.
    ...
    def execute_tool_checked(self, name: str, parameters: dict,
        ...
        then returned an error", so it counts via this method without putting execution metadata
        into ToolResponse.
    ...
    def _select(self, names: Optional[List[str]]) -> List[Tool]:
        """Select all tools when names is None, otherwise existing tools in the requested order.
    ...
    def _param_schema(tool: Tool) -> dict:
        ...
        before execution are the same, so there is zero drift. A complete schema returned by
        ``tool.get_input_schema()`` wins, preserving protocol-level root keywords. Otherwise the
        schema is synthesized from ToolParameter entries as before.
        """
        raw_schema = tool.get_input_schema()
