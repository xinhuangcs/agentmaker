...
class Tool(ABC):
    ...
    Class attributes:
        ...
        supports_parallel: Whether this tool may run concurrently with other tools in the same turn (when the model emits multiple independent read-only
            ...
            (strictly serial, behavior unchanged); setting it True promises "safe under concurrent invocation, no side-effect ordering dependency, does not depend on results
