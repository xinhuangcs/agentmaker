...
class ToolPermissions:
    ...
    def denial_reason(self, tool) -> Optional[str]:
        ...
        Args:
            tool: a tool-name str (judged by name only, backward-compatible) or a Tool object (additionally judged by origin: origin is the root of trust, a name can be spoofed).
        ...
        if name in self.deny:                                          # deny wins: a name match denies immediately
            return self.prompts.render("tool.permission.in_deny", name=name)
        if origin is not None and origin in self.deny_origins:         # an origin match on deny_origins denies immediately
            return self.prompts.render("tool.permission.origin_in_deny", name=name, origin=origin)
        if self.allow is not None or self.allow_origins is not None:   # allowlist enabled: must match either the name or the origin
            ...
            if not (name_ok or origin_ok):
                if self.allow_origins is not None and origin is not None:
                    return self.prompts.render("tool.permission.origin_not_allowed", name=name, origin=origin)
                return self.prompts.render("tool.permission.not_in_allow", name=name)
        return None
