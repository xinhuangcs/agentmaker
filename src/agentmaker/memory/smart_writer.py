...
# constants are convenience aliases snapshotted at import time (they do not follow later DEFAULT_PROMPTS.override;
# for the "currently effective value" use DEFAULT_PROMPTS.text(key) / agent.get_prompts()).
...
class SmartWriter:
    ...
    async def write(self, text: str) -> List[dict]:
        ...
        for fact in await self._extract(text):
            similar = await self.memory.asearch(fact, top_k=self.similar_k)
            decision = {"op": "ADD"} if not similar else await self._reconcile(fact, similar)
            await asyncio.to_thread(self._execute, decision, fact)   # mixed ADD/UPDATE/DELETE operations, run as a whole in a thread pool
    ...
    def _execute(self, decision: dict, fact: str) -> None:
        """Call memory's methods per the decision. UPDATE / DELETE go through soft invalidation rather than physical deletion: the old fact is kept for audit with a supersession chain ("moved house" does not erase "used to live in Shanghai"; physical deletion is reserved for forget / compliance cases, see memory.invalidate)."""
        op = decision.get("op")
        if op == "UPDATE" and decision.get("id"):
            old = self.memory.store.get(decision["id"], scope=self.memory.scope)   # inherit the superseded entry's attributes, don't reset to defaults
            attrs = dict(type=old.type, importance=old.importance, metadata=old.metadata) if old else {}
            new_item = self.memory.add(decision.get("content") or fact, **attrs)   # establish the new fact first (keep importance/type/metadata, only swap the body)
            self.memory.invalidate(decision["id"], superseded_by=new_item.id)      # soft-invalidate the old fact and point it at the successor
        elif op == "DELETE" and decision.get("id"):
            self.memory.invalidate(decision["id"])
        elif op == "NOOP":
            pass
