...
class SqliteCheckpointStore(CheckpointStore):
    ...
    def __init__(self, db_path: str = ":memory:"):
        ...
        try:
            ...
            # Open-time self-check: require_columns must run BEFORE the dedup DELETE / index creation. Both of those
            # GROUP BY / index on the scope columns, so an old DB missing a scope column would first raise a cryptic
            # "no such column" (caught by the except below and generalized), and the clear error with migration
            # guidance would never surface.
            scope_set = set(scope_column_names())
            require_columns(self._db, "checkpoints", scope_set, error_cls=SessionError)
            # An old DB may contain duplicate rows (a legacy artifact of interleaved concurrent DELETE+INSERT): dedup before building the unique index (keep the latest rowid per scope).
            self._db.execute(
                f"DELETE FROM checkpoints WHERE rowid NOT IN "
                f"(SELECT MAX(rowid) FROM checkpoints GROUP BY {cols})")
            # Full-dimension unique index on scope: hand "one point per scope" to the DB to enforce; save uses upsert, so concurrency cannot produce duplicate rows.
            self._db.execute(
                f"CREATE UNIQUE INDEX IF NOT EXISTS idx_checkpoint_scope ON checkpoints({cols})")
            # Unique-constraint self-check runs after index creation (verifies an old DB's existing unique-constraint set covers all scope dimensions; missing columns were already caught by require_columns above).
    ...
    def save(self, state_json: str, *, scope: Optional[Scope] = None) -> None:
        ...
        More robust than the old "DELETE+INSERT as two statements": a single statement has no "deleted but not yet
        inserted" intermediate state, and the unique index means concurrent saves of the same scope only replace and
        never produce duplicate rows (whereas the two interleaved statements could leave two rows).
        """
