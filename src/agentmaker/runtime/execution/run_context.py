...
def check_limits(kind: str) -> None:
    ...
    if p.deadline_seconds is not None and (perf_counter() - ctx.start) > p.deadline_seconds:
        raise RunLimitExceeded(f"exceeded wall-clock time limit {p.deadline_seconds}s")
...
# Governance funnel for bypass LLM calls (governed_chat)
# The framework has a set of direct LLM calls that skip the Harness (SmartWriter extract / compare, Memory summary /
# consolidate, RAG's ask generation and MQE / HyDE query expansion, the LLM Contextualizer). They used to be a
# governance blind spot: not in the trace, not counted against RunPolicy limits. When triggered inside a run via
# MemoryTool / RAGTool, they effectively bypassed the limits. The function below gives them a minimal funnel with the
# same shape as Harness.acall_llm (check_limits -> chat -> record_llm -> optional trace -> hard token limit); called
# outside a run it is automatically a no-op with zero overhead.
