...
# Checkpoint serialization format version: a checkpoint is TRANSIENT data (cleared once resume succeeds), so we do not
# write cross-version migrations and use the cheapest "discard on version mismatch" policy. Bump by 1 when the shape of
# ExecutionState changes (fields added/removed / the meta resume-state structure changes); old checkpoints are then
# recognized, cleared, and the user is prompted to start over, rather than silently misreading an old format as new
# state and resuming (in a HITL scenario this could execute an already-approved action against the wrong state).
# v2: pending changed from a single action to a LIST of actions (supports multiple high-risk actions in one turn /
# batched approval of parallel sub-agents each suspending on their own).
CHECKPOINT_FORMAT_VERSION = 2


def checkpoint_format_version(raw: str) -> Optional[int]:
    """Probe the format version of a checkpoint JSON (missing "v" field = old format -> None); does not fully deserialize, for a pre-load compatibility check."""
    try:
