...
`core/aio` at the paradigm's `run`/`stream_run` synchronous facade layer; the harness exposes only async
operations.
...
from .execution.run_context import (check_deadline, check_limits, correlation,
                                    enforce_token_limit_after_llm, record_llm, record_tool)
...
_StructuredModelT = TypeVar("_StructuredModelT", bound=_StructuredModel)
_ReducedT = TypeVar("_ReducedT", bound=Sized)
...
class HarnessConfig:
    ...
    Excludes hooks and prompts because _make_harness injects self._harness_hooks and self.prompts.
    Tool_registry is supplied per paradigm through _make_harness(tool_registry=...); Reflection's own
    harness intentionally carries none.
    """
...
def cli_confirm(tool, parameters: dict) -> bool:
    ...
    CLI / teaching scenarios pass `confirm=cli_confirm` explicitly. With no confirm callback, registry
    semantics deny the action without reading stdin.
    """
...
def _extract_json(text: Optional[str]) -> str:
    """Extract one JSON value from plain text or a fenced model response."""
    t = (text or "").strip()
    if not t:
        return ""
    fenced = re.search(r"```(?:json)?\s*(.*?)```", t, re.IGNORECASE | re.DOTALL)
    candidates = [fenced.group(1).strip(), t] if fenced else [t]
...
def _validate_structured(
    schema_model: type[_StructuredModelT], content: Optional[str], prompts
) -> tuple[Optional[_StructuredModelT], Optional[str]]:
...
class Harness:
    ...
    def __init__(self, llm: LLMClient, *, tool_registry: Optional[ToolRegistry] = None,
        ...
        Args:
            ...
            window_budget: Optional window-allocation knobs (WindowBudgetConfig); context_block and reduce use it to divide the
                ...
                accounting across retrieval and trajectory budgets.
        ...
        if tool_registry is not None and not callable(getattr(tool_registry, "to_openai_schema", None)):
            raise TypeError(
                ...
                "Common cause: passing system_prompt as the 3rd positional arg to PlanAgent; its signature is (name, llm, system_prompt=None, *, tool_registry=None, ...).")
        self.llm = llm
    ...
    async def astream_llm(self, messages: "list[dict]", **kwargs) -> "AsyncIterator[str | LLMResponse]":
    ...
    async def astructured(self, messages: "list[dict]", schema_model: type[_StructuredModelT], *,
                          retries: int = _DEFAULT_STRUCTURED_RETRIES, **kwargs: Any) -> _StructuredModelT:
    ...
    async def aassemble(self, history, query: str = "", scope=None) -> list[dict]:
        ...
        - When a compactor is attached: overlong history is summary-compacted first (to prevent token buildup; via acompact, with the
          ...
          the app see whether compaction fired and how many turns were summarized).
    ...
    def _window_budget(self):
        ...
        including tool-schema overhead. It is a heuristic estimate, not a strict upper bound: it estimates from the text tool
    ...
    def context_block(self, query: str, scope=None) -> str:
        ...
        The retrieval block and trajectory budgets come from the same WindowBudget allocation.
    ...
    async def areduce(self, kind: str, data: _ReducedT, **kw: Any) -> _ReducedT:
    ...
    def trace_tool_gate(self, name: str, parameters: dict, status: str, detail: str = "") -> None:
        ...
        No latency is recorded because execution did not start. Status is "denied", "rejected", or
        "invalid_args". A no-op with zero overhead when no tracer is attached.
        """
