...
class Hook:
    ...
    def on_run_start(self, input_text: str, *, scope: "Optional[Scope]" = None) -> None | Awaitable[None]:
        """Fires when a run begins (before the input guardrails). input_text is this turn's user input."""

    def before_model(self, messages: "list[dict]") -> None | Awaitable[None]:
        """Fires before each LLM call; messages is the message list about to be sent (also fires for streaming calls)."""

    def after_model(self, response: "LLMResponse") -> None | Awaitable[None]:
        """Fires after a complete LLM response, including the terminal response of a tool-bearing stream."""

    def before_tool(self, name: str, parameters: dict) -> None | Awaitable[None]:
    ...
    def after_tool(self, name: str, parameters: dict, result: "ToolResponse") -> None | Awaitable[None]:
        """Fires after a tool executes; result is a ToolResponse."""

    def on_guardrail_trip(self, stage: str, message: str) -> None | Awaitable[None]:
        """Fires on a guardrail tripwire; stage is "input" / "output", message is the human-readable block explanation."""

    def on_interrupt(self, pendings: "list[PendingAction]", *, scope: "Optional[Scope]" = None) -> None | Awaitable[None]:
    ...
    def on_error(self, error: Exception) -> None | Awaitable[None]:
    ...
    def on_run_end(self, output, *, scope: "Optional[Scope]" = None) -> None | Awaitable[None]:
