...
from ..retrieval.index_sync import IndexSync, SyncIndexSync, call_drop, call_drop_exact
...
def _normalized_utc(dt: datetime) -> datetime:
    """Normalize a non-optional datetime to aware UTC."""
...
class SqliteSessionStore(SessionStore):
    ...
    def __init__(self, db_path: str = ":memory:"):
        ...
        self._coordination_key = (
            None if db_path == ":memory:" else f"sessions:{os.path.realpath(db_path)}")
        ...
        with self._lock:
            try:
                self._db.execute(
                    ...
                    (*sv, message.role, content_value, _normalized_utc(message.timestamp).isoformat(), metadata_json))
                self._db.commit()
            except sqlite3.Error as e:
    ...
    def append_many(self, messages: List[Message], *, scope: Optional[Scope] = None) -> None:
        ...
        rows = [(*sv, m.role, content_value, _normalized_utc(m.timestamp).isoformat(), self._dump_metadata(metadata))
            ...
            try:
                if before is not None:
                    ...
                    cur = self._db.execute(f"DELETE FROM session_messages{clause}",
                                           (*params, _normalized_utc(before).isoformat()))
    ...
    def _dump_metadata(metadata) -> str:
        ...
        Catches TypeError for unsupported values and ValueError for circular references.
        """
        try:
    ...
    def _row_to_message(row) -> Message:
        ...
        return Message(content=content, role=role,
                       timestamp=_normalized_utc(datetime.fromisoformat(created_at)), metadata=metadata_dict)
...
class ConversationSearch(SessionStore):
    ...
    def _feed(self, messages: List[Message], ids: List[str], scope) -> None:
        ...
        mds = [{"role": m.role, "created_at": _normalized_utc(m.timestamp).isoformat()} for m in messages]
    ...
    def append(self, message: Message, *, scope: Optional[Scope] = None) -> None:
        ...
        with self._mutations.hold([scope or Scope()]):
    ...
    def clear(self, *, scope: Optional[Scope] = None, all_scopes: bool = False) -> None:
        """Clear the index before deleting the authoritative session records.

        Messages without a message_id are not present in the index and are skipped.
        """
        guard = (self._mutations.hold_all() if all_scopes
                 else self._mutations.hold([scope or Scope()]))
        with guard:
            ids = {
                message_id
                for message in self.store.load(scope=scope, all_scopes=all_scopes)
                if (message_id := message.metadata.get("message_id"))
            }
            if ids:
                if all_scopes:
                    call_drop(self._sync, sorted(ids), scope=Scope(base="conversation"), strict=True)
                else:
                    call_drop_exact(self._sync, sorted(ids), scope=self._conv_scope(scope), strict=True)
    ...
    def close(self) -> None:
        """Close the source store and index-sync bookkeeping."""
        if self._closed:
            return
        self._closed = True
        errors = []
