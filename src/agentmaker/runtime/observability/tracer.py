...
class Tracer:
    ...
    def emit(self, event: dict) -> None:
        ...
        except Exception as e:  # noqa: BLE001  Cleaning is part of side-channel observation; a throw here must not take down the main flow.
            if self._strict:
                raise
            self._clean_errors += 1
            if self._clean_errors == 1:            # Warn on first failure (with traceback); afterward only count, do not spam.
                _logger.warning("trace event cleaning failed, event dropped (subsequent same-type failures are no longer warned individually, see dropped_uncleanable in tracer.summary): %r",
                                e, exc_info=True)
            return
        for exporter in self.exporters:
            try:
                exporter.export(cleaned)
            except Exception as e:  # noqa: BLE001  Side-channel observation must not take down the main flow; but no longer fully silent: warn on first + count.
                if self._strict:
                    raise
                name = type(exporter).__name__
                if name not in self._export_errors:    # Warn only on the first failure (with traceback); subsequent failures of the same exporter are only counted, not spammed.
                    _logger.warning("trace exporter %s export failed, subsequent same errors are no longer warned individually (see dropped in tracer.summary): %r",
