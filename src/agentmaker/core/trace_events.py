...
Trace events are schema-less free-form dicts, but the `type` field is the contract between them and
third-party exporters / alerting. Previously each producer hardcoded string literals
("llm_call" / "tool_call" / "index_sync_pending", ...) scattered across 5+ files, forcing third-party
consumers to reverse-engineer the set. This module collects every event type into constants that
producers reference uniformly to avoid drift; third parties import it to get a dependable event list.
...
ALL_EVENT_TYPES = frozenset({
    EVENT_LLM_CALL, EVENT_TOOL_CALL, EVENT_CONTEXT_BLOCK, EVENT_CONTEXT_REDUCE, EVENT_CONTEXT_COMPACT,
    EVENT_SUMMARIZE_FAILED, EVENT_MEMORY_SEARCH, EVENT_RAG_RETRIEVE, EVENT_RAG_QUERY_TRANSFORM_FAILED,
    EVENT_RAG_CONTEXTUALIZE_FAILED, EVENT_INDEX_SYNC_PENDING, EVENT_INDEX_SYNC_RECONCILE,
})
