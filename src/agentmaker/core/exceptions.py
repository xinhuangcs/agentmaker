...
All agentmaker exceptions inherit from a common base, AgentmakerError (which inherits RuntimeError for backward compatibility): to catch "any exception the framework raised" at a higher layer, use `except AgentmakerError`, which is more precise than `except RuntimeError` (that would wrongly swallow third-party RuntimeError). All new exceptions hang under AgentmakerError. Concretely there are LLMError (with LLMConfigError / LLMRequestError / LLMResponseError / ContextWindowExceeded underneath) / RetrievalError / SessionError / ToolError / GuardrailTripwireError / RunLimitExceeded / RunCancelled.
"""
