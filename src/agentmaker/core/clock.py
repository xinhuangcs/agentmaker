...
All persisted timestamps across the framework use aware UTC (never naive local time): once written across processes / time zones, ordering stays stable and unambiguous, unaffected by daylight saving time (DST) transitions or the server changing time zone. This module is the shared time source for memory, checkpoints, bookkeeping, tracing, and messages.

When reading a naive stored value or receiving an external naive datetime, ensure_utc normalizes it, avoiding `TypeError: can't compare offset-naive and offset-aware datetimes` when subtracting or comparing aware and naive values.
"""
...
def ensure_utc(dt: Optional[datetime]) -> Optional[datetime]:
    ...
    Used on the read side (fromisoformat restoring a naive stored string, or a caller passing a naive
