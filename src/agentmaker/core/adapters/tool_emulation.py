...
Boundaries: one round parses only a single tool call (no parallel); streaming does no tools (passthrough,
consistent with the framework's "streaming yields text only"); with tools it does not also send a
...
class ToolEmulationAdapter:
    ...
    async def stream(self, messages, *, temperature=None, max_tokens=None, on_stats=None, **kwargs):
        """Streaming does no tool emulation (consistent with the framework's "stream yields text only"): drop tools and pass straight through to the underlying adapter."""
