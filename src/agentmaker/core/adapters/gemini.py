...
class GeminiAdapter(BaseAdapter):
    ...
    def _client_closer(client):
        """Return Gemini's asynchronous transport closer."""
        aio = getattr(client, "aio", None)
        return getattr(aio, "aclose", None) or BaseAdapter._client_closer(client)
            ...
            if role == "assistant" and m.get("tool_calls"):  # model-initiated tool call -> function_call part
                ...
                for tc in m["tool_calls"]:
                    ...
                    if sig is None:
                        ...
                        thought_signature = base64.b64decode(sig) if isinstance(sig, str) else sig
                        parts.append(types.Part(function_call=function_call,
