...
def _to_openai_part(part: dict) -> dict:
    """Translate one neutral content part into Chat Completions format."""
    kind = part.get("type")
    if kind == "text":
...
def _tool_call_dict(tool_call) -> dict:
    """Keep provider extensions while normalizing the callable fields."""
    ...
    call.update({"id": getattr(tool_call, "id", None),
...
class _StreamState(_BaseStreamState):
    ...
    def feed(self, chunk) -> str:
        ...
        for tc in (getattr(ch.delta, "tool_calls", None) or []):
            ...
            if callable(dumped):
                try:
                    raw_extra = dumped(mode="json", exclude_none=True)
                except TypeError:
                    raw_extra = dumped(exclude_none=True)
        ...
        reasoning = getattr(ch.delta, "reasoning_content", None)
        if reasoning:
    ...
    def final_reasoning_content(self):
        """Return the accumulated provider reasoning field, if present."""
...
class OpenAIAdapter(BaseAdapter):
    ...
    def _parse_chat(self, resp, latency: int) -> LLMResponse:
        ...
        try:
            ...
            tool_calls = [_tool_call_dict(tc) for tc in raw_calls] if raw_calls else None
        except LLMError:
            raise
        except Exception as e:  # noqa: BLE001
        ...
        return LLMResponse(
            ...
            assistant_message={"reasoning_content": reasoning} if reasoning is not None else None,
    ...
    async def stream(self, messages, *, temperature=None, max_tokens=None, on_stats=None,
        ...
        if params.get("tools"):
            ...
            reasoning = st.final_reasoning_content()
            yield LLMResponse(content="".join(st.text_parts), finish_reason=st.finish_reason,
                              model=st.model, usage=st.usage, reasoning_content=reasoning,
                              tool_calls=st.final_tool_calls(),
                              assistant_message={"reasoning_content": reasoning} if reasoning is not None else None,
