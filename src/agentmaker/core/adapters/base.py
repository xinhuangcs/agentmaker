...
class BaseAdapter(ABC):
    ...
    def _async_client_for_loop(self, factory):
        ...
        with self._async_clients_lock:
            ...
            if client is None:
                ...
                from ..aio import _register_loop_cleanup
                adapter_ref = weakref.ref(self)

                async def cleanup(owner_loop):
                    adapter = adapter_ref()
                    if adapter is not None:
                        await adapter._aclose_client_for_loop(owner_loop)

                _register_loop_cleanup(("llm-adapter", id(self)), cleanup)
        return client

    @staticmethod
    def _client_closer(client):
        """Return the SDK client's close callable, if any."""
        return getattr(client, "aclose", None) or getattr(client, "close", None)

    async def _aclose_sdk_client(self, client) -> None:
        """Close one SDK client on its owning event loop."""
        closer = self._client_closer(client)
        if closer is None:
            return
        result = closer()
    ...
    async def _aclose_client_for_loop(self, loop: asyncio.AbstractEventLoop) -> None:
        """Remove and close the cached client owned by loop."""
        with self._async_clients_lock:
            client = self._async_clients.pop(loop, None)
        if client is not None:
            try:
                await self._aclose_sdk_client(client)
            except BaseException:
                with self._async_clients_lock:
                    self._async_clients.setdefault(loop, client)
                raise

    async def aclose(self) -> None:
        """Close the SDK client owned by the current event loop."""
        loop = asyncio.get_running_loop()
        from ..aio import _unregister_loop_cleanup
        await self._aclose_client_for_loop(loop)
        _unregister_loop_cleanup(("llm-adapter", id(self)), loop=loop)
