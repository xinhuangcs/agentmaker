...
def _to_anthropic_block(part: dict) -> dict:
    """Translate one neutral content part into an Anthropic content block (verified against
    the official vision guide: image blocks carry a source of type base64 or url)."""
    kind = part.get("type")
    if kind == "text":
...
class AnthropicAdapter(BaseAdapter):
    ...
    def _parse_message(self, resp, latency: int) -> LLMResponse:
        ...
        return LLMResponse(
            ...
            tool_calls=tool_calls or None, latency_ms=latency, raw=resp,
        )
    ...
    async def stream(self, messages, *, temperature=None, max_tokens=None, on_stats=None, **kwargs):
        ...
        try:
            ...
            self.last_stream_stats = stats                  # moved into finally; on a mid-stream exception (final=None) do not keep stale stats from last time
    ...
    def _final_stats(self, final, latency_ms: int) -> StreamStats:
        ...
        return StreamStats(model=getattr(final, "model", self.model),
