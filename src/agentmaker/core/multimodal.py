...
def image_part_from_file(path: Union[str, Path], media_type: str | None = None) -> dict:
...
def content_text(content: MessageContent | None) -> str:
    """Flatten content into plain text: None becomes empty text, str passes through, and part lists join text parts with
...
def content_tokens(content: MessageContent | None, counter: Callable[[str], int] = count_tokens) -> int:
