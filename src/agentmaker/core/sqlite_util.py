...
separate versions for tables with independent schemas within the same file; and since the Scope
...
def unique_column_sets(conn: sqlite3.Connection, table: str) -> List[Set[str]]:
    ...
    Used to detect constraint drift that could let scoped upserts overwrite sibling ownership rows.
    """
    out: List[Set[str]] = []
...
def require_columns(conn: sqlite3.Connection, table: str, expected: Set[str], *,
    ...
    Used by append-only tables without a primary key (such as session_messages): when a database
    is missing required scope-dimension columns, INSERT or exact-match would raise the obscure
    ...
    if missing:
        raise error_cls(
            f"Table {table} is missing columns {sorted(missing)}: detected a database with a "
            "mismatched schema (most likely the set of Scope dimensions changed). "
            "The Scope dimensions are this table's persistence contract; a missing column makes existing "
...
def require_primary_key(conn: sqlite3.Connection, table: str, expected_pk: Set[str], *,
    ...
    overwrite each other across scopes. A primary-key drift such as a single-column key or a key
    that omits a required scope dimension cannot be altered automatically and would leak
...
def require_unique_columns(conn: sqlite3.Connection, table: str, expected: Set[str], *,
    ...
    Used by kv (UNIQUE(scope,key)) and checkpoints (UNIQUE INDEX(scope)): if a database has
    unique-constraint drift that omits a required scope dimension, upserts across that dimension
    would overwrite each other and silently leak data across scopes, so this raises
    loudly at open time.
    """
    if expected not in unique_column_sets(conn, table):
        raise error_cls(
            ...
            f"constraints: {[sorted(s) for s in unique_column_sets(conn, table)]}): detected a "
