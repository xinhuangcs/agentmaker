...
def mmr_select(candidates: List[RetrievalResult], *, top_k: Optional[int] = None,
    ...
    # Note: each round recomputes each candidate's similarity to the selected set,
    # complexity roughly O(n^2 * k). Context candidates are usually only a dozen or so, so
    # it is imperceptible; if the candidate count ever grows large, one could cache "each
    # candidate's current max similarity to the selected set" and update it incrementally
    # after selecting one (deferred, to avoid premature optimization).
    while remaining and len(selected_idx) < limit:
        # First drop candidates that are near-duplicates of an already-selected item
        # (similarity >= threshold); they no longer participate.
        kept_remaining = []
        for i in remaining:
