...
The overall window accounting provides the trajectory token budget (Harness.reduce ->
WindowBudget.trajectory_budget, see window_budget.py). This module applies loss-aware trimming once that
budget is known.
...
def _msg_tokens(m: dict, counter: TokenCounter = count_tokens) -> int:
    """Estimate message content, tool calls, and provider continuation state."""
    if m.get("_provider_payload"):
        return tokens_of(json.dumps(m["_provider_payload"], ensure_ascii=False), counter=counter)
    content = m.get("content")
    n = content_tokens(content if isinstance(content, (str, list)) else None, counter)
    if m.get("tool_calls"):
        n += tokens_of(json.dumps(m["tool_calls"], ensure_ascii=False), counter=counter)
    if m.get("reasoning_content"):
        n += tokens_of(m["reasoning_content"], counter=counter)
    return n
