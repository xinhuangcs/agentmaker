...
class RAGTool(Tool):
    ...
    def __init__(self, pipeline: IngestionPipeline, rag_retriever: RagRetriever, *, top_k: int = 5,
                 filter_fields: tuple = (), scope_policy: Union[str, Callable] = "fixed",
        ...
        Args:
            ...
            scope_policy: ``fixed`` (default), ``merge_run``, or a callable receiving
                ``(fixed_scope, current_run_scope)``. ``merge_run`` fills selected empty dimensions.
            inherit_dimensions: Dimensions inherited by ``merge_run``; conflicts fail loud. Session
                is opt-in so a persistent knowledge base does not become conversation-local by accident.
        """
    ...
    def _scope(self, fixed: Scope) -> Scope:
        """Resolve a pipeline or retriever scope under the explicit tool policy."""
        ...
        if not callable(policy):
            raise TypeError("RAGTool scope_policy must be callable after policy resolution")
        resolved = policy(fixed, runtime)
        if not isinstance(resolved, Scope):
            raise TypeError("RAGTool scope_policy callable must return a Scope")
        return resolved
    ...
    def is_external_content(self, parameters: dict) -> bool:
        """Guard only calls that return knowledge-base content."""
        return (parameters.get("action") or "").strip() in {"search", "ask"}
    ...
    def run(self, parameters: dict) -> ToolResponse:
        ...
        if action == "add_text":
            ...
            pipeline_scope = self._scope(getattr(self.pipeline, "scope", Scope(base="rag")))
            res = self.pipeline.ingest_text(text, source="agent_input", fmt=fmt, scope=pipeline_scope)
        ...
        if action == "add_document":
            ...
            pipeline_scope = self._scope(getattr(self.pipeline, "scope", Scope(base="rag")))
            res = self.pipeline.ingest_file(path, scope=pipeline_scope)
        ...
        if action == "search":
            ...
            retrieval_scope = self._scope(getattr(self.rag_retriever, "scope", Scope(base="rag")))
            hits = self.rag_retriever.retrieve(query, top_k=self.top_k, scope=retrieval_scope,
        ...
        if action == "ask":
            ...
            if not query:
                return ToolResponse.error(self.prompts.render("rag.msg.need_query", action=action))
            retrieval_scope = self._scope(getattr(self.rag_retriever, "scope", Scope(base="rag")))
            return self._format_ask(run_sync(self.rag_retriever.ask(
                query, top_k=self.top_k, scope=retrieval_scope,
                filters=self._filters(parameters))))

        if action == "stats":
            pipeline_scope = self._scope(getattr(self.pipeline, "scope", Scope(base="rag")))
            s = self.pipeline.stats(scope=pipeline_scope)
    ...
    async def arun(self, parameters: dict) -> ToolResponse:
        ...
        if action == "ask":
            ...
            if not query:
                return ToolResponse.error(self.prompts.render("rag.msg.need_query", action=action))
            scope = self._scope(getattr(self.rag_retriever, "scope", Scope(base="rag")))
            return self._format_ask(await self.rag_retriever.ask(query, top_k=self.top_k, scope=scope,
                                                                  filters=self._filters(parameters)))
