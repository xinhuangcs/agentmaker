...
DEFAULT_ASK_PROMPT = DEFAULT_PROMPTS.text("rag.ask")   # Now supplied by the central agentmaker.prompts registry; kept as a convenience alias.
...
DEFAULT_MQE_PROMPT = DEFAULT_PROMPTS.text("rag.mqe")   # Now supplied by the central registry; kept as a convenience alias.
...
DEFAULT_HYDE_PROMPT = DEFAULT_PROMPTS.text("rag.hyde")   # Now supplied by the central registry; kept as a convenience alias.
...
class RagRetriever:
    ...
    def from_config(cls, config, *, embedder=None, retriever: Optional[HybridRetriever] = None,
        ...
        return cls(retriever, source_store if source_store is not None else SourceStore(db_path), llm,   # is not None: do not get tripped up by a custom store whose __bool__ is False.
    ...
    def retrieve(self, query: str, *, top_k: Optional[int] = None, scope: Optional[Scope] = None,
        ...
        eff_scope = scope or self.scope  # The same scope is used both for retrieval and for fetching chunks from the source-of-truth store, avoiding sibling chunks from a different scope.
        ...
        if orphans:
            try:
                self.retriever.delete(orphans, scope=eff_scope)   # Self-heal on read: best-effort cleanup of index orphans (same as memory.search).
            except Exception as e:  # noqa: BLE001  Cleanup is a bonus; failing to delete must not drag down this retrieval; leave a debug trace.
                logging.getLogger(__name__).debug("Failed to clean up retrieval index orphans (does not affect this retrieval): %r", e)
    ...
    def _search(self, query: str, *, top_k: int, scope: Scope, filters=None) -> List[RetrievalResult]:
        ...
        # RetrievalError; the single-query path previously omitted this, and would crash when top_k
        # exceeded the default candidate_pool. Align with the multi-query path / Memory.search by
        # passing max(...).
        pool = max(self.cfg.candidate_pool, top_k)
        if self.query_transformer is None:
            return self.retriever.search(query, top_k=top_k, candidate_pool=pool, scope=scope, **fkw)
        ...
        if len(queries) == 1:
            return self.retriever.search(queries[0], top_k=top_k, candidate_pool=pool, scope=scope, **fkw)
        ...
        result_lists = self.retriever.search_many(   # Embed all queries in a single batch, saving N-1 round trips.
            queries, top_k=per_query, candidate_pool=max(self.cfg.candidate_pool, per_query), scope=scope, **fkw)
        # Cross-query fusion goes through the backend's single fusion seam (once a weighted fusion is injected, the multi-query path no longer uses hardcoded RRF); if the backend does not expose fusion (a custom stub), fall back to RRF.
    ...
    async def ask(self, query: str, *, top_k: Optional[int] = None, scope: Optional[Scope] = None, filters=None) -> "AskResult":
        ...
        answer = (await governed_chat(self.llm, self._build_messages(query, chunks),
    ...
    async def ask_stream(self, query: str, *, top_k: Optional[int] = None, scope: Optional[Scope] = None, filters=None):
        ...
        if not chunks:
            ...
            return
        async for piece in self.llm.stream(self._build_messages(query, chunks)):
            yield piece
