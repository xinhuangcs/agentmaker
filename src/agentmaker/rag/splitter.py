...
class Splitter(ABC):
    ...
    def _pack(self, units: List[str], doc: Document, *, heading_path: str = "",
        ...
        idx = start_index
        for unit in units:
            if buf and self._count("\n".join([*buf, unit])) > self.chunk_tokens:
                chunks.append(self._make(buf, doc, heading_path, idx))
                idx += 1
                buf = self._tail_overlap(buf)
                while buf and self._count("\n".join([*buf, unit])) > self.chunk_tokens:
                    buf.pop(0)
            buf.append(unit)
        if buf:
            chunks.append(self._make(buf, doc, heading_path, idx))
    ...
    def _char_split(self, text: str) -> List[str]:
        ...
        while text:
            ...
            while lo <= hi:
                mid = (lo + hi) // 2
                if self._count(text[:mid]) <= self.chunk_tokens:
                    best, lo = mid, mid + 1
                else:
                    hi = mid - 1
            if best == 0:
                raise ValueError("token_counter reports one character larger than chunk_tokens")
            out.append(text[:best])
            text = text[best:]
        return out

    def _tail_overlap(self, buf: List[str]) -> List[str]:
        ...
        itself exceeds overlap_tokens, do not overlap, to avoid stuffing a large unit into the next
