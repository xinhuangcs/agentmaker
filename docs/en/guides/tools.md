...
Decorate a type-annotated function with `@tool` and it becomes a `Tool` object (no longer a plain function), ready to pass to `Agent(tools=[...])` or `registry.register(...)`. The schema the model sees is inferred for you:
...
- `supports_parallel`: set `True` for read-only, concurrency-safe tools so they may run concurrently with other parallelizable calls in the same turn.
...
You can override the dangerous-argument gate with the `arg_policy` callback, and the subprocess environment with `env`.
...
`NotesTool` uses action-level confirmation: `append` writes to disk and requires confirmation, while `read` is read-only and runs without a confirmation prompt.
...
- Each tool definition gets a fingerprint (a sha256 over its remote name, description, and input schema). Pass `expected_fingerprints` to pin them: if the server later swaps a tool's description or schema, `load_tools` refuses to load.
