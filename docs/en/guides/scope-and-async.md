...
Every dimension must be a string or `None`. Empty strings normalize to `None`; numeric or boolean identifiers raise `TypeError` instead of silently becoming an unrestricted dimension. Convert numeric application identifiers explicitly, for example `Scope(user=str(user_id))`.

`Memory`, `RagRetriever`, and `IngestionPipeline` always enforce their canonical `base` (`memory` or `rag`). A per-call scope may replace the other dimensions, but a missing base is filled automatically and a conflicting base raises. Thus `Scope(user="alice")` cannot accidentally cross subsystem indexes.
...
One Agent instance serializes the full run / resume / stream lifecycle for an exact scope. Concurrent calls from the same event loop queue on that scope; a contending call from another event loop (typically two threads calling the synchronous facade at once) fails with `SessionError` instead of waiting on a lock owned by the wrong loop. A later, non-overlapping call from another thread is allowed after the scope becomes idle. This gate is local to one Agent instance and process; applications sharing a checkpoint or session backend across processes still need their own coordination.

Stateful tools do not silently inherit the Agent run scope. `MemoryTool` and `RAGTool` default to `scope_policy="fixed"`. In a shared multi-tenant Agent, opt in with `scope_policy="merge_run"`; it fills empty `user`, `agent`, and `app` dimensions and rejects conflicts with fixed values. `session` is deliberately opt-in through `inherit_dimensions=...`, because long-term memory and knowledge bases normally outlive one conversation. A callable policy can implement application-specific ownership.
...
`llm.stream(...)` is an async generator of text deltas. Without tools it yields strings only; with tools, an adapter that supports native streaming function calls additionally yields one final `LLMResponse` after the text drains (the channel the agent's streaming tool loop consumes). Text tool emulation is non-streaming, so use `run` / `arun` rather than `stream_run` / `astream_run` when `emulate_tools=True`. After a stream finishes you can read `llm.last_stream_stats` for that call's usage, latency, and finish reason.
