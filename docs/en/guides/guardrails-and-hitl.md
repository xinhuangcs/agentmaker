...
| `after_model(response)` | After each non-streaming LLM call; `response` is an `LLMResponse`. |
...
- **Crash recovery**: state is saved every step, so after a process restart `resume()` (no decision) continues from the last checkpoint.
...
- `deadline_seconds`: wall-time limit measured from the start of the run; must be `> 0`.
