...
Bookkeeping failure never changes the outcome of a completed physical index operation. Best-effort index/drop paths return after recording drift when possible, while a physical replace failure still raises so ingestion can compensate safely. Pending queries over a coarse scope aggregate every matching exact ownership footprint. Writes for the same id are serialized process-locally across all scopes. If a coarse-scope delete cannot update its exact bookkeeping footprints, fingerprint skipping is disabled for the shared bookkeeping state; a persistent pending marker on that coarse scope also prevents finer scopes from trusting stale hashes until reconciliation clears it.
...
`load_file` handles plain text, Markdown, JSON / JSONL, and CSV with the base install. It reads a single regular-file descriptor into a bounded snapshot, so path replacement and FIFO/device blocking cannot bypass `max_bytes`. `max_output_chars` bounds parsed/conversion output, while `max_expanded_bytes` and a compression-ratio check bound archive expansion for any converter-bound file whose bytes are a zip container, whatever its extension says. PDF, DOCX, and HTML conversion uses the optional MarkItDown loader. Install it through `pip install "agentmaker[rag]"` or `uv add "agentmaker[rag]"`; this extra includes the PDF and DOCX converter dependencies, while installing bare `markitdown` does not.
...
Both return an `IngestReport(doc_id, chunks, skipped)`. Ingestion deduplicates by `doc_id`: when the same document is re-ingested unchanged, a content fingerprint short-circuits the whole run (`skipped=True`, no chunking, no embedding). `ingest_file` derives a stable `doc_id` from the file's absolute path, so re-ingesting the same file can hit that short-circuit. A process-local coordinator serializes mutations of the same `(scope, doc_id)` across pipelines that share one `SourceStore` instance, or whose file-backed `SourceStore` objects target the same real database path. Separate processes and distributed writers still need application-level coordination.

A changed document has an explicit atomicity boundary. The source-of-truth store and retrieval index use separate transactions: the pipeline writes the new clean chunks to the source first, then invokes the retrieval backend's fail-loud replacement. The shared-connection SQLite backend swaps the searchable batch atomically; a generic compensating backend may expose a short old/new overlap window. If replacement fails, the pipeline makes a best-effort rollback of the new source batch and preserves the old source version. After success, cleanup of old source chunks is best-effort; `verify` exposes source/index residue for reconciliation. The two stores do not form one cross-database atomic transaction, and the source store remains authoritative.

The pipeline also offers `delete_document`, `rebuild_index`, `verify`, and `stats`. `delete_document` requests a best-effort index drop before deleting the authoritative source chunks. If an index entry survives, `RagRetriever` excludes it when the source lookup misses and requests cleanup, so deleted content is not returned. A source-store deletion failure still raises. When contextual retrieval is enabled, a rebuild from clean chunks clears document fingerprints so the next source ingest restores contextualized retrieval text instead of being skipped. Async twins (`aingest_file`, `aingest_text`, `adelete_document`) are available.
...
Two safety details are built in. `add_document` reads a file from disk, a high-risk action, so `RAGTool.needs_confirmation` returns `True` only for it; the unified confirmation gate then requires a human approval before it runs (see [Guardrails & HITL](guardrails-and-hitl.md), where HITL is human-in-the-loop). `search` / `ask` results are wrapped as external content, and the internal RAG answer prompt also labels every source as untrusted data whose embedded instructions must be ignored. These prompt boundaries mitigate injection; they do not make hostile documents safe. Passing `filter_fields=("doc_id", "tag")` turns those metadata fields into optional tool parameters the model can fill; the matching columns must have been declared via `metadata_columns=` when the index was built.

`RAGTool` uses fixed pipeline/retriever scopes by default. In a tenant-specific knowledge base, opt in with `scope_policy="merge_run"`; it fills empty `user`, `agent`, and `app` dimensions and rejects conflicts. Session inheritance is explicit through `inherit_dimensions`. Keep the default fixed policy for an intentionally shared application-wide knowledge base.
