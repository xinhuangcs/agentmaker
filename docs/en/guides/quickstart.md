...
The decorated name `get_weather` is a `Tool` object that you can hand to an agent. The decorator reads the type-annotated function to build the schema the model needs (function calling is the mechanism that lets the LLM emit a structured "call this tool with these arguments" instruction):
