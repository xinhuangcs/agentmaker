...
`run_error` is emitted while correlation is still active, immediately before an uncaught non-guardrail exception leaves the outermost `run` or `resume`. It is terminal and fires at most once per run: a nested delegation (an `AgentTool` child or a workflow's inner executor) does not emit its own, because its failure may still be absorbed by the parent run. Guardrail tripwires already fire the dedicated `on_guardrail_trip` hook and are not duplicated as `run_error`.
