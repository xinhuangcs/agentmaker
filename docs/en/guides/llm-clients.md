...
!!! note
    Model names and endpoints can change. Treat the table as the shipped defaults, not a permanent guarantee. Use `gemini` / `anthropic` (the native protocols) when you want each vendor's full native feature set; `gemini_openai` is the OpenAI-compatible shim for Gemini.

### Registered model limits

The profile limits apply only to a provider's default model. When you pass another registered model ID, `LLMClient` resolves its model-specific context window and maximum output instead. The relevant exact token limits are:

| Model ID(s) | Context window | Maximum output |
| --- | ---: | ---: |
...
| `kimi-k2.7-code`, `kimi-k2.6`, `kimi-k2.5` | 262,144 | not registered |
| `moonshot-v1-8k` | 8,192 | 8,192 |
| `moonshot-v1-128k` | 131,072 | not registered |

Moonshot publishes no independent output cap (a call may produce at most the window minus the prompt), so the registry leaves Kimi K2.x and `moonshot-v1-128k` output unset and your `max_tokens` applies as passed. DeepSeek's documented "384K" output expands to 393,216 (binary K). DashScope still defaults to the cheaper `qwen-flash`; select `qwen3.6-flash` explicitly when that model is intended.
...
The SDK client is cached per event loop. Use `async with LLMClient(...)` or call `await llm.aclose()` when your application owns the client. Synchronous code can use `with LLMClient(...)` together with `run_sync`; `close()` releases that thread's SDK client. The synchronous bridge also closes registered clients, pending tasks, and its resident loop when a temporary worker thread exits.
...
Without tools, every yielded piece is a string. With a native tool schema passed as `tools=`, adapters that support streaming function calls yield the text deltas and then exactly one terminal `LLMResponse`; that response carries the accumulated `tool_calls` used by the Agent's streaming tool loop. Text tool emulation does not emulate calls on the streaming path: `ToolEmulationAdapter` streams plain text, and an Agent configured with `emulate_tools=True` rejects a streaming tool loop. Use non-streaming `chat` / `run` / `arun` for emulated tools.

### Stream statistics

Per-call stream metadata lives separately from the yielded text and optional terminal tool response. After the stream drains, read `llm.last_stream_stats` (or `None` if you have not streamed yet). It exposes `model`, `finish_reason`, `usage`, and `latency_ms`. For OpenAI-family providers the client automatically requests streaming usage (`stream_options={"include_usage": True}`), so `usage` is normally populated; it can still be `None` for backends that never report usage on streams.
...
Every non-streaming call returns an `LLMResponse` dataclass. A stream invoked with tools also emits one terminal `LLMResponse` after its text deltas. The fields:
...
| `assistant_message` | `dict \| None` | JSON-safe provider continuation state for a tool turn. The Agent feeds it back automatically. |
...
For models that lack native function calling, `LLMClient(..., emulate_tools=True)` wraps the adapter with a text-emulation shim so tool-using agents work through non-streaming `chat` / `run` / `arun`. It does not support streaming tool loops. Enable it only when native function calling is unavailable, since emulation is less reliable and costs extra tokens. See [Tools](tools.md) for the tool system itself.
