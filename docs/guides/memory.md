...
| `id` | Unique identifier, auto-generated (a uuid) unless you set it. |
...
- `invalidate(id, superseded_by=...)` soft-invalidates a memory: it leaves the index and is no longer recalled, but the record is kept for audit. This is what `SmartWriter` uses.
- `delete(id)` / `delete_many(ids)` physically remove memories from both the store and the index (use this for compliance erasure).
