...
`llm.stream(...)` 是一个产出文本增量的异步生成器。不带工具时它只产出字符串；当传入工具时，它会在文本流耗尽后额外产出一个最终的 `LLMResponse`（也就是 Agent 的流式工具循环所消费的那条通道）。一次流式结束后，你可以读取 `llm.last_stream_stats` 来获取该次调用的用量、延迟和结束原因。
