...
`run_error` 会在关联上下文仍然有效时产出，时机是一个未捕获的非护栏异常即将离开最外层 `run` 或 `resume` 之前。它是终态事件，每次运行至多一条：嵌套委托（`AgentTool` 子代理或工作流的内部执行器）不会各自产出，因为它的失败可能仍被父运行消化掉。护栏触发线已经触发专门的 `on_guardrail_trip` hook，因此不会再重复记为 `run_error`。
