...
def test_tool_status_messages_go_through_prompts():
    ...
    Covers the dependency-free error paths of calculator / search / notes / shell / rag / memory / tool_search / conversation_search,
    proving the hardcoded-Chinese consolidation actually reached the code (not just added registry entries).
    """
    import re
    cjk = re.compile(r"[一-鿿　-〿]")
    ...
    notes = NotesTool("/tmp/notes_i18n", prompts=reg)
...
def test_full_english_switch_leaves_no_chinese():
    ...
    for tool in (CalculatorTool(prompts=reg), SearchTool(prompts=reg),
                 NotesTool("/tmp/n", prompts=reg), CLITool([], prompts=reg)):   # empty allowlist exercises tool.none
        treg.register(tool)
