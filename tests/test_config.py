"""Agentmaker configuration-system tests (hermetic: no key, offline).
...
def test_known_models_catalog_resolution():
    ...
    current = LLMClient("openai", model="gpt-5.6-luna", api_key="x")
    assert current.context_window == 1_050_000 and current.max_output_tokens == 128_000
    # default model unaffected by the catalog (still takes the profile value)
    assert LLMClient("openai", api_key="x").context_window == 1_047_576
    ...
    # a registered model resolves through an OpenAI-compatible custom endpoint too
    k = LLMClient("openai_compatible", model="kimi-k2.6", api_key="x", base_url="http://x/v1")
    assert k.context_window == 262_144 and k.max_output_tokens is None   # Moonshot publishes no independent output cap
...
def test_from_dict_bool_string_and_non_mapping():
    assert AgentmakerConfig.from_dict({"context": {"allow_borrow": "false"}}).context.allow_borrow is False
